...
Java_com_example_ryan_1ffmpeg_1player_VideoView_render(JNIEnv *env, jobject thiz, jstring input,
                                                       jobject surface) {
    ...
    while (av_read_frame(pFormatCtx, packet) >= 0) {
        ...
        if (packet->stream_index == videoIndex) {
            ...
            if (!got_picture) {
                ...
                //4 输入数据第一列要转码的位置 从0开始
                //5 输入画面的高度
                int scaleResult = sws_scale(sws_ctx, frame->data, frame->linesize, 0, frame->height,
                          rgb_frame->data, rgb_frame->linesize);

                LOGI("scaleResult: %d ", scaleResult);
